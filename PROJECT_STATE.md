...
- Model, Mirror, and Hologram now share one logo-free application header with the `MimickMe™` title and `digital fashion smart mirror` eyebrow. The Model preview occupies the same right-hand stage position as the Mirror avatar, with model-source and visibility controls overlaid inside the black stage. Former monospace labels now use lowercase Geist Sans Medium (500), and Geist Mono is no longer loaded.
