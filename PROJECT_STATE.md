...
- Model, Mirror, and Hologram now share one logo-free application header with the `MimickMe™` title and `Digital fashion smart mirror` eyebrow. Model-source and visibility controls are overlaid inside the black Model stage. Former monospace labels now use sentence-case Geist Sans Medium (500), and Geist Mono is no longer loaded.
- Model and Mirror now use the same 20px page frame as Hologram. The interactive Model stage is the first, left-hand panel and the inspector/context panel sits on the right. The stage keeps setup controls at bottom-left, its interaction hint at top-right, and validation plus the Smart Mirror entry action at bottom-right; responsive layouts move secondary actions only when needed to avoid covering the avatar. Mirror calibration uses the same dark overlay treatment and exposes a live waiting, positioning, calibrating, or calibrated status tag.
